...
if (isset($_POST['delete'])) {
    $data = $_POST;
    $curl = curl_init('http://localhost/uni_comm/api/courses/delete.php');
    curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode($data));
    $flag = json_decode(curl_exec($curl));
    echo $flag ? "<script>alert('Deleted.')</script>" : "<script>alert('Error')</script>";
}
                                                ...
                                                <div class="d-flex align-items-center justify-content-between">
                                                    ...
                                                    <div class="d-flex align-items-center pl-4">
                                                        ...
                                                        <?php if($_SESSION['user']['type'] == ADMIN || $_SESSION['user']['type'] == TEACHER) : ?>
                                                                <button class="btn btn-secondary" style="margin-left:80px" data-toggle="modal" data-target="#uploadNewFile">
